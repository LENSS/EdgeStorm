...
public class StatusReporterEKBased implements Runnable {
    ...
    public void addTaskForMonitoring(BTask task) {
    ...
    public void reportAppStatusOfLocalTasksToEdgeKeeper(){
        ...
        for (Map.Entry<Integer,String> taskEntry: StatusOfLocalTasks.task2Component.entrySet()) {
            ...
            if(StatusOfLocalTasks.task2EmitTimesUpStream.get(tid)!=null && StatusOfLocalTasks.task2EmitTimesUpStream.get(tid).size()!=0){
                ...
                reportOfLocalTasks.task2InputRate.put(tid, Double.valueOf(String.format("%.4f", inputRate)));
                reportOfLocalTasks.task2OutputRate.put(tid, Double.valueOf(String.format("%.4f", outputRate)));
                reportOfLocalTasks.task2ProcRate.put(tid, Double.valueOf(String.format("%.4f", procRate)));
                reportOfLocalTasks.task2SojournTime.put(tid, Double.valueOf(String.format("%.4f", sojournTime)));
                reportOfLocalTasks.task2InQueueLength.put(tid, Double.valueOf(String.format("%.4f", inQueueLength)));
                reportOfLocalTasks.task2OutQueueLength.put(tid, Double.valueOf(String.format("%.4f", outQueueLength)));
                ...
                String report = "Time: "           + String.format("%.0f", curTime)        + ","
                              + "TaskID: "         + tid                                   + ","
                              + "Component: "      + component                             + ","
                              + "InputRate: "      + String.format("%.4f", inputRate)      + ","
                              + "OutputRate: "     + String.format("%.4f", outputRate)     + ","
                              + "ProcRate: "       + String.format("%.4f", procRate)       + ","
                              + "SojournTime: "    + String.format("%.4f", sojournTime)    + ","
                              + "InQueueLength: "  + String.format("%.4f", inQueueLength)  + ","
                              + "OutQueueLength: " + String.format("%.4f", outQueueLength) + "\n";
                try {
