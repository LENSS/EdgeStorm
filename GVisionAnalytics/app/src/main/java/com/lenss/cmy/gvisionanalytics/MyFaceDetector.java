                        ...
                        case AFD:   //// Android Media Face Detection Library, around 100ms
                            ...
                            for (int j = 0; j < face_detected; ++j) {
                                ...
                                try {
                                    ...
                                    int x2 = (int) Math.min(face.getPosition().x + face.getWidth(), bitmap.getWidth());
                                    int y2 = (int) Math.min(face.getPosition().y + face.getHeight(), bitmap.getHeight());
                                    bmface = Bitmap.createBitmap(bitmap, x, y, x2-x, y2-y);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                    continue;
                                }
...
//                        // not distributed CNN model
//                        String compAge = MyAgeAnalyzer.class.getName();
//                        String compGender = MyGenderAnalyzer.class.getName();
//                        String compEmotion = MyEmotionAnalyzer.class.getName();
//                        try {
...
//                        }
                        ...
                        String compFaceCommonFeature = MyFaceCommonFeature.class.getName();
                        try{
                            MessageQueues.emit(pktSend, taskID, compFaceCommonFeature);
                        } catch (InterruptedException e){
                            e.printStackTrace();
                        }

                    }
                    logger.info("FACE DETECTOR CATCHES FACES, "+SystemClock.elapsedRealtimeNanos());
                }
            }
        }
    }
